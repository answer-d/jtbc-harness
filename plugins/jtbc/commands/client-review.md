---
description: 客先レビュー(お客様確認)を実施する。社内の内部承認(ゲート)を得た成果物を、課長がお客様へご提示し、ご承認を賜る工程。ご承認をもって次フェーズへ進む。引数: <gate_name>
argument-hint: "<proposal|project_plan|basic_design|detailed_design>"
---

# /jtbc:client-review

**社内の内部承認(ゲート)を経た** 成果物を、課長(お客様窓口)がお客様(ユーザー)へ
**正式にご提示** し、**ご確認・ご承認を賜る** 工程です。

> **順序が重要**: 内部承認前の文書をお客様に出してはいけません。
> 必ず `/jtbc:gate <gate>` で **上位承認(社内審査)** を得てから、本コマンドでお客様へ提示します。
> 流れ: ①課長が起案 → ②`/jtbc:gate <gate>`(内部承認)→ ③`/jtbc:client-review <gate>`(客先提示・ご承認)→ ④次フェーズへ
...
2. **内部承認チェック(最重要)**: `modes/jtbc.yaml#gates[<gate>].approvers` の **全員** が
   ...
   - 未承認(=社内審査が未了)の場合は **中止**し、
     「お客様へご提示する前に、社内の承認手続き(/jtbc:gate <gate>)が必要です」と内部向けに案内する
...
9. 結果による分岐:
   ...
   - **ご指摘あり・要修正(REVISION_REQUESTED)** → `status: "REVISION_REQUESTED"`、`feedback` に記録。
     ...
     担当部署(課長/主任)が成果物を修正し、**再度 `/jtbc:gate <gate>`(内部承認)→ `/jtbc:client-review <gate>`** の順で進める。

> **phase 遷移の所在**: `internal_approval_first` のゲートでは、phase を進めるのは **gate ではなく
> 本コマンド(client-review APPROVED 時)**。内部承認(gate)は phase を進めない。

## 出力例 (ご提示〜ご査収のお願い)

```
🤝 要件定義書 客先レビュー  (担当: 課長 / 社内承認済み)
...
→ 課長が要件定義書を修正 → /jtbc:gate project_plan(再・内部承認)→ /jtbc:client-review project_plan の順で進めます。
```
