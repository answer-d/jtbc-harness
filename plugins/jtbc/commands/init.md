...
6. **キックオフ**: **営業(メインセッション=lead)** が応対する。まず D-4 に従い **ご相談内容を確認** する
   ...
   - **teams モード(`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`)**: 営業が一言ご挨拶のうえ **課長を常駐
     teammate として spawn** し、お客様を **課長へお繋ぎ**(「ここからは弊社PMの課長が直接ヒアリングいたします」)。
     以降の1問ずつのヒアリングは **課長 teammate がお客様と直接対話** して行い、**リードは spawn 後に退いて中継しない**。
     - **spawn の仕方(一発実行にしない)**: `Agent` を **`run_in_background: true`** + **`name:"課長"`** +
       `agentType:"jtbc:jtbc-kacho"` で呼ぶ。❌ `subagent_type` だけの一発実行(`Done` で返る形)にしない
       (中継になりバグ再発)。spawn プロンプトに **「対話型ヒアリング。1問だけ尋ねたら止まってお客様の返答を待て。**
       **自走しない・先に提案書を書かない。要件が固まってから起案」** と明記する。
     - **split-pane(tmux/iTerm2)が前提**(課長ペインで直接会話。in-process は `Shift+Down` で課長へ切替)。
     - **お繋ぎ後、リードは黙って待つ(誤検知の防止・最重要)**: 課長↔お客様の対話はリードから見えない。
       `Teammate ... came to rest`(idle)は **正常**(返答待ち)であり故障ではない。idle を理由に問題検知・
       緊急停止しない。課長へ催促 SendMessage しない。「SendMessage が無い」等の自己診断で問題報告に走らない。
       お客様へ案内は1回だけ、以降は沈黙。**提案書(`.jtbc/proposal/proposal.md`)が起案されるまで内部審査へ進まない**。
     - 課長はヒアリング結果を **自分の文脈のまま** 提案書に起案する(引き渡し不要)。起案後、team-lead へ完了を
       知らせる(使えなければお客様にリードのペインへお戻りいただくようご案内)。
...
お問い合わせいただき、誠にありがとうございます。JTBC で本件の窓口を担当いたします営業でございます。
ご相談の「<判明している依頼内容>」につきまして、ここからは弊社で本件のPMを務めます課長が
