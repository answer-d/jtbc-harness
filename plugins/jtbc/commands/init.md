...
6. **キックオフ**(テンプレ配置に続けて **同一ターンで必ず実行**): **営業(メインセッション=lead=Human Gateway)** が応対する。まず D-4 に従い
   ...
   **【提案ゲートは社長承認も必須】** 提案書がまとまったら社内審査(提案ゲート)を行うが、ここは
   **部長に加えて社長(`jtbc-shacho`)の承認も必須**(`state_guard` が `bucho`+`shacho` 両方の押印を要求)。
   司令塔は部長だけで済ませず **社長を必ず起こして審査・押印を得る**。承認が欠けて移行が deny されても
   **PMO に承認を代筆させて迂回しない** ―― 社長を起こして本人に押印させるのが唯一の正解(詳細は governance)。
