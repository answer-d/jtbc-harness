...
6. **キックオフ**: 提案に向けたご要望のヒアリングを始める旨を、`customer-relations`
   トーンでお客様へお伝えする(下記出力)。ヒアリングは `requirements-interview` スキル
   (`/jtbc:hearing`)の手順に従い、**質問を羅列せず1問ずつ・推奨案を添えて** 伺う。
   提案フェーズは課長が主担当(規模が大きければ主任に一部オフロード可)。
   **受注の御礼はここでは述べない**(提案ご承認時に申し上げる)。
...
`requirements-interview` に従い、**質問を羅列せず1問ずつ・推奨案を添えて** お伺いする:
...
【ご確認 1/?】 本システムで「これができたら成功」と言える、いちばんの狙いはどこにございますか。
  弊社としては「<推奨案>」を中核と想定しておりますが、ご認識に相違ございませんでしょうか。
```
...
次のステップ:
  1. ご要望のヒアリング(/jtbc:hearing で1問ずつ。課長が提案書を起案いたします)
  2. /jtbc:gate proposal           # 提案審査(社内でお受けするか審査=上位承認。客先提示の前提)
  3. /jtbc:client-review proposal   # 内部承認済みの提案書をお客様へご提示・ご承認(ご承認時に受注の御礼)
