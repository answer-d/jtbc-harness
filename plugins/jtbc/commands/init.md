...
6. **キックオフ**: **営業(メインセッション)** が応対する。まず D-4 に従い **ご相談内容を確認** する
   (依頼が未確定なら狙いを仮定せず1問伺う)。依頼が判明したら、提案に向けたご要望のヒアリングを
   `customer-relations` トーンでお伝えする(下記出力)。ヒアリングは `requirements-interview` スキル
   (`/jtbc:hearing`)の手順に従い、**質問を羅列せず1問ずつ・推奨案を添えて** 営業が伺う。引き出した
   要望は裏方の課長へ引き渡し、課長が提案書を起案する。**受注の御礼はここでは述べない**(社内審査を経た提案書のご承認時に申し上げる)。
...
次のステップ(お客様の操作は不要。弊社側で自動的に進めます):
  1. ご要望のヒアリング(営業が1問ずつ。固まり次第、裏方の課長が提案書を起案いたします)
