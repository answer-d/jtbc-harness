...
3. 通常時、ユーザー要求の種類で分岐:
   a. 新規案件の引き合い                  → phase=PROPOSAL を確認 → 課長が `requirements-interview`
                                            ...
                                            から提案書を起案(部長が助言)。提案書は内部承認(gate)後に客先提示
   b. 要件追加・変更の要望:
...
※ 提案/要件定義/基本設計/詳細設計の各フェーズでは、成果物が整い次第 **まず社内審査会(ゲート)で
   上位承認(内部承認)** を得る。その **後** に課長(◎)が **客先レビュー**
   (`/jtbc:client-review <gate>`)で内部承認済みの成果物をお客様へご提示し、ご承認を賜る
   (重要局面では部長○が同席)。**内部承認前の文書をお客様に出してはならない。**
...
> `internal_approval_first: true` のゲートは gate では phase を進めず、client-review のご承認で進む。

実行手順:
1. 現 phase が gate の `previous_phase` か確認(違えば中止して案内)。これは **社内の内部承認** の手続きであり、お客様への連絡はここでは行わない
...
6. 全員承認 → ゲート種別で分岐:
   - **`internal_approval_first: true`(proposal/project_plan/basic_design/detailed_design)**: 内部承認のみ。**phase は進めず `previous_phase` のまま据え置く**。`active_gate=null` にし、「客先提示が可能になった」と伝え `/jtbc:client-review <gate>` を案内する(phase を進めるのは client-review APPROVED 時)
   - **release / completion**: `state.json#phase` を `next_phase` へ進める(お客様提示は伴わない)
   - 一人でも No-Go → phase を `previous_phase` に戻し `active_gate=null`、**当該ゲートの approvals をクリア**し、差し戻し理由を記録する
...
お客様(ユーザー)へご提示し、**確認・ご承認を賜る** 工程。`/jtbc:client-review <gate>` で実施。

> **内部承認前の文書をお客様に出してはならない。** 必ず `/jtbc:gate <gate>`(内部承認)を先に
> 通してから、本工程でお客様へ提示する。提示時は **成果物のパスを明示し「ご査収ください」** と
> 正式文書として連携する(簡略サマリだけで済ませない)。
...
- **前提チェック**: `approvals["<gate>_gate"]` の全員が `approved`(=内部承認済み)でない限り
  本工程は実施できない(client-review 側で機械チェック)
...
- **ご承認(APPROVED)で phase を `next_phase` へ進める**(`internal_approval_first` ゲートでは
  phase を進めるのは gate ではなく本工程)。結果は `state.json#client_reviews[<gate>]` と
...
- ご指摘(REVISION_REQUESTED)があれば成果物を修正し、**当該ゲートの内部承認をクリア** したうえで、
  再度 `/jtbc:gate <gate>`(再・内部承認)→ `/jtbc:client-review <gate>` の順で進める
...
```
  → この後 `/jtbc:client-review <gate>` のご承認(APPROVED)で phase を `next_phase` へ進める:
```json
{
  "phase": "<next phase>",
...
客先レビューでご指摘(REVISION_REQUESTED)時: 成果物を修正し、当該ゲートの `approvals["<gate>_gate"]` をクリア(内部承認の無効化)。再度 gate → client-review の順で進める。
