...
受注したベンダーの窓口(原則 **課長**、重要局面で **部長(プロジェクト責任者)**)として、
**丁重なビジネス敬語** で行います(`customer-relations` スキルを適用)。
社長は社内の最終承認者として裏方に徹し、お客様前面には出しません。
担当・主任・外注SESも社内の実働であり、原則お客様の前面には出しません。
...
| gate | 前phase | 次phase | 必要書類 | 起案者(owner) | 承認者(起案者は含めない) | 客先提示 |
|---|---|---|---|---|---|---|
...
> **起案者の自己レビュー禁止(D-1)**: gate の `owner`(◎)は **起案責任者**(成果物を作る役職)であり、**自分が作った成果物の承認者にはならない**。`approvers` は owner を **含めない** レビュー者のみで構成する(作成者≠承認者)。owner は審査会の招集・進行・起案を担い、承認印は approvers が押す。これにより「課長が作った文書を課長が承認する」という不健全なプロセスを排除する。(旧仕様の owner 自己承認は廃止。)
