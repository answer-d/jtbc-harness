...
### 【最重要】お客様と直接対話させた後の lead の振る舞い (誤検知の防止)

> 課長(等)へお客様をお繋ぎしたら、その対話は **teammate のペインで お客様↔teammate の間** で進む。
> **lead からは見えない**。lead はここで暴走しがちなので、以下を厳守する:

- **`Teammate @xxx came to rest`(idle 通知)は正常**。teammate が1問尋ねてお客様の返答を待っている
  状態であり、**故障ではない**。idle 通知を理由に「問題検知」「緊急停止」「プラグイン不具合」と判断しない。
- **teammate へ催促の `SendMessage` を送らない**(進行中の対話を妨げる)。お客様↔teammate で自律的に進む。
- **idle 通知に逐一返答・中継しない**。lead は **退いて静かに待つ**。
- お客様へは **1回だけ**「課長のペインへお切り替えください(in-process は `Shift+Down`)。ヒアリング完了まで
  こちらはお待ちします」とご案内し、以降は沈黙する。
- **「SendMessage が無い/中継できない」等の自己診断で問題報告に走らない**。pane 直接対話に中継は不要。
- **再開の合図**(これが来るまで内部審査へ進まない): teammate が「ヒアリング完了・提案書起案済み」を知らせる
  (`SendMessage` で team-lead へ通知できれば通知)。**通知が使えない場合の確実な検出は
  `.jtbc/proposal/proposal.md` の生成確認**。お客様が lead のペインへ戻られたら、lead は提案書の存在を
  確認して内部審査(自動開催)へ進む。
- **spawn プロンプトに完了合図を入れる**: ヒアリング役の teammate には「完了し提案書を起案したら、
  team-lead へ完了を知らせ(使えなければお客様にlead のペインへお戻りいただくようご案内)せよ」と指示する。
