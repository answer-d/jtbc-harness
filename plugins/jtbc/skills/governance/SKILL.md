...
3. 通常時、ユーザー要求の種類で分岐:
   a. 新規案件の引き合い                  → phase=PROPOSAL を確認 → **営業(リード=Human Gateway)が直接
                                            ヒアリング**(`requirements-interview` に従い1問ずつ・推奨案つき)。
                                            お客様との生対話はリードが務め、teammate には渡さない(下記「Human Gateway」)。
                                            共通理解が取れたら、合意内容を **裏方の課長(teammate / フォールバックは
                                            サブエージェント)へ渡して提案書を起案** させる(部長が助言)。
...
- **spawn の仕方**: `Agent` ツールを `run_in_background: true` + `name` 付きで呼び、`agentType` に
  `jtbc:jtbc-<role>` を指定すると常駐 teammate になる(記憶が継続)。並行で複数役職に作業させたいとき
  (例: 複数承認者の並行押印)に有効。一発実行(`subagent_type` 指定で `Done` が返る)でもよいが、その場合は
  記憶が継続しない。**いずれにせよユーザー対話はリードが担い、teammate はユーザーに直接話しかけない**。
