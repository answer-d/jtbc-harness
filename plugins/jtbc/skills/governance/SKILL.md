...
3. 通常時、ユーザー要求の種類で分岐:
   a. 新規案件の引き合い                  → phase=PROPOSAL を確認 → 営業(メインセッション)が
                                            `requirements-interview` スキル(`/jtbc:hearing`)で要望を1問ずつ
                                            引き出し、共通理解を得る。確定した要望を裏方へ渡し、**課長(常駐 teammate)が
                                            提案書を起案**(部長が助言)。起案後は **下記「内部審査の自動開催」**
