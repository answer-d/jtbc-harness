...
- **【重要】phase 遷移の実行者は PMO**: `state.json#phase` を書けるのは **PMO(`jtbc-pmo`)のみ**(`state_guard` が物理担保)。
  ...
  特に **project_plan ゲートは計画書・要件定義書・リスク登録簿・WBS骨子が実内容で埋まっていること**(WBS は
  PMBOK「WBS の作成」=計画プロセス群の成果物。スコープ・ベースラインの一部として計画時に必須)を PMO が確認し、
...
4. 課長が要件定義書・計画書・リスク登録簿・WBS骨子を作成(PMO はプロセス検証)→ (自動)PJ計画審査(計画書+要件+リスク+WBS骨子が必須)→
